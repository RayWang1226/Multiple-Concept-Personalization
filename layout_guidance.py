...
def layout_guidance_sampling(seed,
    ...
    register_attention_control(unet, max_length)
    ...
    for i, t in tqdm(enumerate(scheduler.timesteps), total=len(scheduler.timesteps)):
        ...
        for k in range(loop):
            if i < guidance_steps:
                ...
                cfg_factor = (1 + scheduler.sigmas[i] / scheduler.sigmas.max())  # Adjust based on sigma
                dynamic_cfg_scale = cfg_scale * cfg_factor  # Dynamic CFG scaling
                noise_pred = noise_pred_uncond + dynamic_cfg_scale * (noise_pred_text - noise_pred_uncond)
...
def _extract_cross_attention(tokenizer, device, color_map_image, color_context, text_input, neg):
    ...
    cross_attention_weight_2 = _tokens_img_attention_weight(
        seperated_word_contexts, text_input, ratio=2
    ).to(device)
    cross_attention_weight_4 = _tokens_img_attention_weight(
        seperated_word_contexts, text_input, ratio=4
    ).to(device)
    ...
    hidden_states = {
        ...
        "CA_WEIGHT_147456": cross_attention_weight_2,
        "CA_WEIGHT_36864": cross_attention_weight_4,
